...
Ubuntu 20.04 Focal Fossa as the OS.
...
# Install R packages
sudo R -e 'install.packages(read.table("https://gist.githubusercontent.com/dmolitor/f88b3edd1fd02716ff08888a815db35a/raw/8298cc7d28aa0c03a7e0330e338fa7c85d0ee782/r-packages.txt")[["V1"]], Ncpus = parallel::detectCores())'
```
...
## Bonus Stuff (Configuring Basic Authentication)

Obviously, setting up some form of basic authentication will depend on your
...
Once you are in the file, you will tweak the existing server function as
follows:
```
server {
...
auth_basic on;
```
Now, save and exit the file. Next you need to create a `username` user and 
...
```
You have now established basic authentication and when you access any page on
