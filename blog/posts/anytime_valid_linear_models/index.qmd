...
Now, for a dataset comprised of these units $\{(X_i, W_i, Y_i)\}_{i=1}^N$
let's construct a function that will estimate both $\hat{\tau}_{DM}$ and
$\hat{\tau}_{CA}$ as well as the corresponding exact and asymptotic CSs.
...
Now, let's run our simulation for N = 500 and plot $\hat{\tau}_{DM}$,
$\hat{\tau}_{CA}$, and their CSs at each step
...
We will run create a new function to generate data from this experimental setup
and will run our simulation for N = 2000. Same as above, we'll plot $\hat{\tau}_{IPW}$,
$\hat{\tau}_{CAIPW}$, and their CSs at each step.
...
We see that, indeed, Type 1 error is $\leq \alpha$.